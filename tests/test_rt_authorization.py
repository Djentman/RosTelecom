...
class TestValidRegistrationRT:
    ...
    def test_clicable_navi_bar(self):
        WebDriverWait(self.driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, RTPanelNaviBar.LOCATOR_NAVI_BAR_LS))
        ).click()
        assert self.driver.find_element(By.XPATH, RTPanelNaviBar.LOCATOR_FORM_LS)
        WebDriverWait(self.driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, RTPanelNaviBar.LOCATOR_NAVI_BAR_MAIL))
        ).click()
        assert self.driver.find_element(By.XPATH, RTPanelNaviBar.LOCATOR_FORM_MAIL)
        WebDriverWait(self.driver, 10).until(
            EC.element_to_be_clickable(
                (By.XPATH, RTPanelNaviBar.LOCATOR_NAVI_BAR_LOGIN)
            )
        ).click()
        assert self.driver.find_element(By.XPATH, RTPanelNaviBar.LOCATOR_FORM_LOGIN)
        WebDriverWait(self.driver, 10).until(
            EC.element_to_be_clickable(
                (By.XPATH, RTPanelNaviBar.LOCATOR_NAVI_BAR_TELEPHONE)
            )
        ).click()
    ...
    def test_autorization_invalid_email(self):
        self.driver.find_element(
            By.ID, RTAutorizationLocators.LOCATOR_RT_AUTORIZATION_USER
        ).send_keys(Invalid_Data.fake_email)
        self.driver.find_element(
            By.ID, RTAutorizationLocators.LOCATOR_RT_AUTORIZATION_PASSWORD
        ).send_keys(Invalid_Data.fake_password)
        self.driver.find_element(
            By.ID, RTAutorizationLocators.LOCATOR_RT_AUTORIZATION_BUTTON_LOGIN
        ).click()
        assert self.driver.find_element(
            By.XPATH, RTAutorizationAllerts.LOCATOR_RT_AUTORIZATION_ALLERTS_ERROR
        )
        assert self.driver.find_element(
            By.XPATH, RTAutorizationAllerts.LOCATOR_ERROR_TEXT_INVALID_EMAIL
        )
    ...
    def test_autorization_invalid_phoneNumber(self):
        self.driver.find_element(
            By.ID, RTAutorizationLocators.LOCATOR_RT_AUTORIZATION_USER
        ).send_keys(Invalid_Data.invalid_phoneNumber)
        self.driver.find_element(
            By.ID, RTAutorizationLocators.LOCATOR_RT_AUTORIZATION_PASSWORD
        ).send_keys(Invalid_Data.fake_password)
        self.driver.find_element(
    ...
    def test_autorization_invalid_password(self):
        self.driver.find_element(
            By.ID, RTAutorizationLocators.LOCATOR_RT_AUTORIZATION_USER
        ).send_keys(Valid_Data.valid_phoneNumber)
        self.driver.find_element(
            By.ID, RTAutorizationLocators.LOCATOR_RT_AUTORIZATION_PASSWORD
        ).send_keys(Invalid_Data.fake_password)
        self.driver.find_element(
    ...
    def test_autorization_xss_in_login(self):
        ...
        assert WebDriverWait(self.driver, 10).until(
            EC.visibility_of_element_located(
                (By.XPATH, RTAutorizationAllerts.LOCATOR_ERROR_TEXT_XSS)
            )
        )

# python -m pytest -v test_rt_authorization.py --alluredir reports
